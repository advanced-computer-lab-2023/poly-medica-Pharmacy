...
import { ADMIN_ENUM, ADMIN_SIGNUP_URL, BAD_REQUEST_CODE_400, DUB_EMAIL_ERROR_MESSAGE, DUB_USERNAME_ERROR_MESSAGE, DUPLICATE_KEY_ERROR_CODE, OK_REQUEST_CODE_200, ONE_DAY_MAX_AGE_IN_MIINUTS, ONE_DAY_MAX_AGE_IN_MILLEMIINUTS, PATIENT_ENUM, PATIENT_SIGNUP_URL, PHARMACIST_BASE_URL, PHARMACIST_ENUM, PHARMACIST_SIGNUP_URL, SERVER_ERR_REQUEST_CODE_500 } from "../utils/Constants.js";
...
export const user = (app)=>{
    ...
    app.post('/signup', async (req, res) =>{
        try{
            ...
            switch(type){
                ...
                case PHARMACIST_ENUM: email = req.body.userData.email; userName = req.body.userData.userName;break;
            ...
            await axios.post(`${PHARMACIST_BASE_URL}check-pharmacist-req`, {email, userName});
    ...
    app.delete('/users/:id', async (req,res)=>{
        try{
            ...
            res.status(SERVER_ERR_REQUEST_CODE_500).send({ errMessage: "coudn't delete the user"});
        }
    });
    ...
    app.post('/pharmacists', async (req, res) => {
        try{
            ...
            res.status(SERVER_ERR_REQUEST_CODE_500).send({errMessage:"coudn't add the doctor"})
        }
    });
    ...
    app.post('/admins', async (req, res) =>{
        try{
            ...
            const signupData = await axios.post(); // TODO: the end point for admin add user <= here
            ...
            if(err.response){ // coming from other services
                if(err.response.data.errCode == DUPLICATE_KEY_ERROR_CODE){
                    res.status(BAD_REQUEST_CODE_400).send({message: err.response.data.errMessage});
                }
                else res.status(BAD_REQUEST_CODE_400).send({message:err.response.data.errMessage});
            } else{ // coming from this services
                res.status(BAD_REQUEST_CODE_400).send({message: err.message});
            }
        }
    })
