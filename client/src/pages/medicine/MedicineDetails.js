...
const MedicineDetails = ({ selectedMedicine, handleDialogClose }) => {
	return (
		<Dialog open={selectedMedicine} onClose={handleDialogClose} PaperProps={{ sx: { minWidth: window.outerWidth > 800 ? 500 : 300 } }}  >
			{selectedMedicine && (
				<>
					<DialogTitle align='center' variant='h2'>{selectedMedicine.name}</DialogTitle>
					<DialogContent>
						<div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
							<img src={`${PHARMACY_BASE_URL}/medicines/${selectedMedicine._id}/pictures`} alt={selectedMedicine.name} width="180" height="180" style={{ margin: 'auto' }} />
						</div>
						...
						<Typography variant="body1">{selectedMedicine.quantity}</Typography>
